...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    {
        have_init_last_position_ctrl = true;
        last_position_ctrl =  position_ctrl;
    ...
    if(is_command_mode && is_hover_mode &&  position_ctrl > POSITION_CTRL_VALUE)
    {
        is_position = true;
    }
    else
...
void Odom_Data_t::feed(nav_msgs::OdometryConstPtr pMsg, Imu_Data_t *imu_data)
{
    ...
    imu_data->q.x() = msg.pose.pose.orientation.x;
    imu_data->q.y() = msg.pose.pose.orientation.y;
    imu_data->q.z() = msg.pose.pose.orientation.z;
    imu_data->q.w() = msg.pose.pose.orientation.w;
// #define VEL_IN_BODY
