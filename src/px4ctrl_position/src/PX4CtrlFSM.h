...
public:
    ...
    enum State_t
    {
        ...
        AUTO_TAKEOFF,
        AUTO_LAND,
        QR_CODE_RECOGNITION, // 新增状态：识别二维码
        FLY_TO_OBSTACLE,     // 新增状态：飞行至障碍物处
        CIRCLE_OBSTACLE,     // 新增状态：绕障碍物飞行
        DROP_OBJECTS,        // 新增状态：投放物块
        PASS_THROUGH_RING,   // 新增状态：穿越圆环
        FLY_TO_LANDING_POINT // 新增状态：飞行至降落点
    };
...
private:
    ...
    void publish_position_ctrl(const ros::Time &stamp);
    void publish_trigger(const nav_msgs::Odometry &odom_msg);

    // 新增函数声明
    Desired_State_t get_qr_code_recognition_des();
    Desired_State_t get_fly_to_obstacle_des();
    Desired_State_t get_circle_obstacle_des();
    Desired_State_t get_drop_objects_des();
    Desired_State_t get_pass_through_ring_des();
    Desired_State_t get_fly_to_landing_point_des();

    // 新增辅助变量和函数
    Eigen::Vector3d obstacle_position;
    std::vector<Eigen::Vector3d> drop_positions;
    Eigen::Vector3d ring_position;
    Eigen::Vector3d landing_point;
    
    // 当前正在处理的投放点索引
    int current_drop_index;

    bool recognize_qr_code();
    bool reach_obstacle();
    bool finish_circle_obstacle();
    bool finish_drop_objects();
    bool pass_through_ring();
    bool reach_landing_point();

    // 新增用于图片靶识别相关的函数声明（目前仅为预留，实际功能未完整实现）
    bool recognize_image_target(const cv::Mat& frame, const std::string& target_class);
    Eigen::Vector3d get_image_target_position(const std::string& target_class);
};
